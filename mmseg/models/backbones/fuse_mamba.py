    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if cascade2d:
            ...
            xs = CrossScan.apply(x)  # (b, 4, dim, h*w)
            if no_einsum:
...
class FusionSS2D:
    def __fuseinitv1__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v32dc=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, cascade2d=True),
            # v32dc=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, cascade2d=True),
            fusev1_v05=partial(self.forward_fuse, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
            # fusev2_v05=partial(self.forward_fuse, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
        )
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        k_group = 4
        ...
        # in proj =======================================
        d_proj = d_inner if self.disable_z else (d_inner * 2)
        self.in_proj_a = Linear(d_model, d_proj, bias=bias)
        self.in_proj_b = Linear(d_model, d_proj, bias=bias)
        self.act: nn.Module = act_layer()
        
        # conv =======================================
        if self.with_dconv:
            self.conv2d_a = nn.Conv2d(
                in_channels=d_inner,
                out_channels=d_inner,
                groups=d_inner,
                bias=conv_bias,
        ...
        # out proj =======================================
        self.out_act_a = nn.GELU() if self.oact else nn.Identity()
        self.out_act_b = nn.GELU() if self.oact else nn.Identity()
        self.out_proj_a = Linear(d_inner, d_model, bias=bias)
        self.out_proj_b = Linear(d_inner, d_model, bias=bias)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
    ...
    def forward_fuse(
        ...
        **kwargs,
        ...
        Ds = self.Ds
        delta_softplus = True
        out_norm_a = getattr(self, "out_norm", None)
        out_norm_b = getattr(self, "out_norm", None)
        channel_first = self.channel_first
        to_fp32 = lambda *args: (_a.to(torch.float32) for _a in args)
        ...
        def merge_tensors(tensor1, tensor2):
            merged_tensor = torch.cat((tensor1[:, :2], tensor2[:, 2:]), dim=1)
            # merged_tensor = torch.cat((tensor2[:, 0, ...], tensor1[:, 1, ...], tensor1[:, 2, ...], tensor1[:, 3, ...]), dim=1)
            return merged_tensor
        
        xs_a = CrossScan.apply(x_a)  # (b, 4, dim, h*w)
        xs_b = CrossScan.apply(x_b)
        
        xsm_a = merge_tensors(xs_a, xs_b)
        xsm_b = merge_tensors(xs_b, xs_a)
        
        x_dbl_a = F.conv1d(xsm_a.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K)
        dts_a, Bs_a, Cs_a = torch.split(x_dbl_a.view(B, K, -1, L), [R, N, N], dim=2)
        dts_a = F.conv1d(dts_a.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K)
        
        x_dbl_b = F.conv1d(xsm_b.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K)
        dts_b, Bs_b, Cs_b = torch.split(x_dbl_b.view(B, K, -1, L), [R, N, N], dim=2)
        dts_b = F.conv1d(dts_b.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K)

        xs_a = xs_a.view(B, -1, L)
        xs_b = xs_b.view(B, -1, L)
        
        dts_a = dts_a.contiguous().view(B, -1, L)
        As = -torch.exp(A_logs.to(torch.float)) # (k * c, d_state)
        Bs_a = Bs_a.contiguous().view(B, K, N, L)
        Cs_a = Cs_a.contiguous().view(B, K, N, L)
        Ds = Ds.to(torch.float) # (K * c)
        delta_bias = dt_projs_bias.view(-1).to(torch.float)
        
        dts_b = dts_b.contiguous().view(B, -1, L)
        # As = -torch.exp(A_logs.to(torch.float))
        Bs_b = Bs_b.contiguous().view(B, K, N, L)
        Cs_b = Cs_b.contiguous().view(B, K, N, L)

        if force_fp32:
            xs_a, dts_a, Bs_a, Cs_a = to_fp32(xs_a, dts_a, Bs_a, Cs_a)
            xs_b, dts_b, Bs_b, Cs_b = to_fp32(xs_b, dts_b, Bs_b, Cs_b)

        ys_a: torch.Tensor = selective_scan(
            xs_a, dts_a, As, Bs_a, Cs_a, Ds, delta_bias, delta_softplus
        ).view(B, K, -1, H, W)
        ys_b: torch.Tensor = selective_scan(
            xs_b, dts_b, As, Bs_b, Cs_b, Ds, delta_bias, delta_softplus
        ).view(B, K, -1, H, W)
        
        y_a: torch.Tensor = CrossMerge.apply(ys_a)
        y_b: torch.Tensor = CrossMerge.apply(ys_b)

        y_a = y_a.view(B, -1, H, W)
        y_b = y_b.view(B, -1, H, W)
        if not channel_first:
            y_a = y_a.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1) # (B, L, C)
            y_b = y_b.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_a = out_norm_a(y_a)
        y_b = out_norm_a(y_b)
    ...
    def forwardfuse(self, a: torch.Tensor, b: torch.Tensor,**kwargs):
        x_a = self.in_proj_a(a)  # linear
        x_b = self.in_proj_b(b)
        if not self.disable_z:
            x_a, z_a = x_a.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            x_b, z_b = x_b.chunk(2, dim=(1 if self.channel_first else -1))
            if not self.disable_z_act:
                z_a = self.act(z_a)
                z_b = self.act(z_b)
        if not self.channel_first:
            x_a = x_a.permute(0, 3, 1, 2).contiguous()
            x_b = x_b.permute(0, 3, 1, 2).contiguous()
        if self.with_dconv:
            x_a = self.conv2d_a(x_a) # (b, d, h, w)
            x_b = self.conv2d_b(x_b) # (b, d, h, w)
        x_a = self.act(x_a)
        x_b = self.act(x_b)
        
        y_a, y_b = self.forward_core(x_a, x_b)   # TODO
        y_a = self.out_act_a(y_a)
        y_b = self.out_act_b(y_b)
        if not self.disable_z:
            y_a = y_a * z_a
            y_b = y_b * z_b
        out_a = self.dropout(self.out_proj_a(y_a))
        out_b = self.dropout(self.out_proj_b(y_b))
        return out_a, out_b
...
class FusionSS2Dv2:
    def __fuseinitv2__(
        ...
        **kwargs,    
        ...
        self.with_dconv = d_conv > 1
        Linear = Linear2d if channel_first else nn.Linear
        self.forward = self.forwardfusev2
        ...
        FORWARD_TYPES = dict(
            ...
            v32dc=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, cascade2d=True),
            # v32dc=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, cascade2d=True),
            fusev2_v05=partial(self.forward_fusev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
        )
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        k_group = 4
    ...
    def forwardfusev2(self, a: torch.Tensor, b: torch.Tensor,**kwargs):
        x_a = self.in_proj_a(a)  # linear
        x_b = self.in_proj_b(b)

        if not self.disable_z:
            x_a, z_a = x_a.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            x_b, _ = x_b.chunk(2, dim=(1 if self.channel_first else -1))
            if not self.disable_z_act:
                z_a = self.act(z_a)
                # z_b = self.act(z_b)
        if not self.channel_first:
        ...
        y_a = self.forward_core(x_a)   
        y_b = self.forward_core(x_b)
        y_a = self.out_act_a(y_a)
        y_b = self.out_act_b(y_b)
        if not self.disable_z:
            y_a = y_a * z_a            # TODO
...
class SS2D(nn.Module, mamba_init, SS2Dv0, SS2Dv2, SS2Dv3, FusionSS2D, FusionSS2Dv2):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
        ...
        **kwargs,
        ...
        elif forward_type.startswith("fusev1"):
            self.__fuseinitv1__(**kwargs)
        elif forward_type.startswith("fusev2"):
            self.__fuseinitv2__(**kwargs)
        else:
...
class FusionVSSBlock(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            self.norm_B = norm_layer(hidden_dim)
        ...
        if self.mlp_branch:
            _MLP = Mlp if not gmlp else gMlp
            self.norm2_A = norm_layer(hidden_dim)
            self.norm2_B = norm_layer(hidden_dim)
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
            self.mlp_A = _MLP(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer, drop=mlp_drop_rate, channels_first=channel_first)
            self.mlp_B = _MLP(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer, drop=mlp_drop_rate, channels_first=channel_first)

    def _forward(self, A: torch.Tensor, B: torch.Tensor):
        if self.ssm_branch:
            identity_a = A
            identity_b = B
            A = self.norm_A(A)
            ...
            A = identity_a + self.drop_path(a)
            B = identity_b + self.drop_path(b)
    ...
    def forward(self, input):
        ...
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input_A, input_B)
        else:
            return self._forward(input_A, input_B)
        

class FusionVSSBlockv2(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        dpr_fuse = [x.item() for x in torch.linspace(0, drop_path_rate, sum(fuse_depths))]
        ...
        if self.data == 'hunan':
            # print('==========================================================in emb=================')
            # print(in_chans)
            self.patch_embed_A = _make_patch_embed(in_chans - 2, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
            self.patch_embed_B = _make_patch_embed(2, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        else: 
            self.patch_embed_A = _make_patch_embed(in_chans - 1, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
            self.patch_embed_B = _make_patch_embed(1, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        ...
        self.fuse_layersv2 = nn.ModuleList()
        for i_layer in range(self.num_layers):
            downsample_a = _make_downsample(
                self.dims[i_layer], 
                self.dims[i_layer + 1], 
            ...
            self.layers_a.append(self._make_layer(
                dim = self.dims[i_layer],
                drop_path = dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
                downsample=downsample_a,
                channel_first=self.channel_first,
                # =================
            ...
            self.layers_b.append(self._make_layer(
                dim = self.dims[i_layer],
                drop_path = dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
                downsample=downsample_b,
                channel_first=self.channel_first,
                # =================
                ssm_d_state=ssm_d_state,
                ssm_ratio=ssm_ratio,
            ...
            self.fuse_layers.append(self._make_fuse_layer(
                dim = self.dims[i_layer],
                drop_path = dpr[sum(fuse_depths[:i_layer]):sum(fuse_depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
                channel_first=self.channel_first,
                # =================
            ...
            self.fuse_layersv2.append(self._make_fuse_layerv2(
                dim = self.dims[i_layer],
                drop_path = dpr[sum(fuse_depthsv2[:i_layer]):sum(fuse_depthsv2[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
                channel_first=self.channel_first,
                # =================
    ...
    def _make_fuse_layerv2(
        ...
        **kwargs,
        ...
        blocks = []
        for d in range(depth):
            blocks.append(FusionVSSBlockv2(
                hidden_dim=dim, 
                drop_path=drop_path[d],
                norm_layer=norm_layer,
                channel_first=channel_first,
...
class Fusion_VSSM(VSSM):
    def __init__(self, 
        ...
        super(Fusion_VSSM, self).__init__(**kwargs)
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])
        _NORMLAYERS = dict(
            ln=nn.LayerNorm,
            ln2d=LayerNorm2d,
        ...
        for i in out_indices:
            layer_a = norm_layer(self.dims[i])
            layer_b = norm_layer(self.dims[i])
            layer_name_a = f'outnorm_a{i}'
            layer_name_b = f'outnorm_b{i}'
            self.add_module(layer_name_a, layer_a)
            self.add_module(layer_name_b, layer_b)
    ...
    def forward(self, x):
        ...
        def fuse_forward(l, x, y):
            x, y = l.blocks((x, y)) 
            return x, y
        if self.data == 'hunan':
            A, B = torch.split(x, [13, 2], dim=1)
        else:
            A, B = torch.split(x, [4, 1], dim=1)
        A = self.patch_embed_A(A)
        B = self.patch_embed_B(B)
        outs = []
        
        for i, (layer_a, layer_b, fuse_layer, fusev2) in enumerate(zip(self.layers_a, self.layers_b, self.fuse_layers, self.fuse_layersv2)):

            o_a = layer_a.blocks(A)
            o_b = layer_b.blocks(B)
            
            a, b = fuse_forward(fuse_layer, o_a, o_b)
            a, b = fuse_forward(fusev2, a, b)
            o_a = o_a + a
            o_b = o_b + b   
            
            A = layer_a.downsample(o_a)
            B = layer_b.downsample(o_b)
                     
            if i in self.out_indices:
                norm_layer_a = getattr(self, f'outnorm_a{i}')
                norm_layer_b = getattr(self, f'outnorm_b{i}')
                out_a = norm_layer_a(o_a)
                out_b = norm_layer_b(o_b)

                # out_a, out_b = fuse_forward(fuse_layer, out_a, out_b)
                out = out_a + out_b
